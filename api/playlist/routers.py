...
async def get_my_playlists(
    ...
    for playlist in playlists:
        ...
        tracks_count_result = await session.execute(select(func.count()).where(PlaylistTrack.playlist_id == playlist.id))
...
async def get_playlist(
    ...
    if playlist.user_id != current_user.id and not playlist.is_public:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет доступа к этому плейлисту")
    ...
    tracks_list = []
    for track in tracks:
        song_result = await session.execute(select(Song).where(Song.id == track.song_id))
...
async def update_playlist(
    ...
    if playlist.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет прав для редактирования этого плейлиста")
...
@router.delete("/{playlist_id}/cover", status_code=status.HTTP_200_OK)
async def delete_playlist_cover(
    ...
    playlist.cover = None
    await session.commit()
    return {"status": "success", "message": "Обложка плейлиста удалена"}
...
async def add_track_to_playlist(
    ...
    song_result = await session.execute(select(Song).where(Song.id == track_data.song_id))
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Песня уже добавлена в плейлист")

    max_pos_result = await session.execute(select(func.max(PlaylistTrack.position)).where(PlaylistTrack.playlist_id == playlist_id))
...
async def update_track_position(
    ...
    if not track:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Трек не найден в плейлисте")
    ...
    return {
        ...
        "data": {"id": track.id, "position": track.position}
    }
...
async def remove_track_from_playlist(
    ...
    return {"status": "success", "message": "Трек успешно удален из плейлиста"}
...
async def delete_playlist(
    ...
    if not playlist:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Плейлист не найден")
    if playlist.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Нет прав для удаления этого плейлиста")

    await session.delete(playlist)
    await session.commit()
    return {"status": "success", "message": "Плейлист успешно удален"}
